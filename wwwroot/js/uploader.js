...
function progressHandler(event) {
    ...
    var elem = $('#my-progress-bar');
    if (elem.length>0) {

        elem.get(0).style.width = Math.round(percent) + '%';
        elem.attr('aria-valuenow', Math.round(percent));
        elem.html(Math.round(percent) + "%");
    }
}
...
function fileUploadCompleted(result){
    ...
    for (var i = 0; i < resArr.length; i++) {
        ...
        if (headerText)//begin header of reply card
            strLi += `<div class="card" data-id="` + (replyid ? replyid : "") + `" >
                ...
                `</div>`;
        if (IsImage(resArr[i]))
            strLi += `<label><a data-url="/Home/GetFile?fileName=` + resArr[i] + `&fileType=up" class="img-tag" ><div><img class="fupload" src="/Home/GetFile?fileName=` + resArr[i] + `&fileType=up&th=true" /></div><div>` + SimpleFileName(resArr[i]) + `</div></a></label>`;
        else if (IsAudio(resArr[i]))
            strLi += `<label><div><audio controls src="/Home/GetFile?fileName=` + resArr[i] + `&fileType=up" ></audio></div><div> ` + SimpleFileName( resArr[i]) + `</div></label>`;
        else if (IsVideo(resArr[i]))
            strLi += `<label><div><video controls  ><source src="/Home/GetFile?fileName=` + resArr[i] + `&fileType=up" ></video></div><div> ` + SimpleFileName( resArr[i]) + `</div></label>`;
        else if (IsPdfOrDoc(resArr[i]))
            strLi += `<label><a href="/Home/GetFile?fileName=` + resArr[i] + `&fileType=up" target="_blank" ><div><img class="docpdfimg" src="/Home/GetFile?fileName=` + resArr[i].substring(0, resArr[i].lastIndexOf('.')) + `.jpg&fileType=up&th=true"  /></div><div>` + SimpleFileName(resArr[i]) + `</div></a></label>`;
        else
            strLi += `<label><a href="/Home/GetFile?fileName=` + resArr[i] + `&fileType=up" target="_blank" ><div><img src="/img/file-icon.png" class="img-ico" /></div><div>` + SimpleFileName(resArr[i]) + `</div></a></label>`;
    ...
    var isGroup = $(".contact.active").hasClass('grp');
    if (isGroup)
        sendBySignalRToGroup(contactId, "isfile;;;" + userid + ";;;" + result + ',;,' + headerText + ";;;" + ";;;" + replyid);
    else
    ...
    var elem = $('#my-progress-bar');
    if (elem.length > 0) {
        elem.get(0).style.width = '100%';
        setTimeout(function () {
            elem.get(0).style.width = '0%';
