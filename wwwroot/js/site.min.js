function showStatus(){$("#status-options").css("opacity","1");$("#status-options").css("visibility","visible")}function searchContact(){var n=$("#search input").val();n?($(".new-search-mess").hide(),$(".new-search-mess").removeClass("new-search-mess").addClass("search-mess"),$(".new-search-mess").html(""),$.post("/Home/GetAccounts",{searchKey:n},function(n){var t,r,i;if($(".def-mess").hide(),n.length==0){i=`<li class="contact new-search-mess">
        ...
        `+r+`
            ...
            </li>`;$("#lastchat").before(r);$(".chat-pm ul").scrollTop($(".chat-pm ul").get(0).scrollHeight);setTimeout(function(){$(".chat-pm ul").scrollTop($(".chat-pm ul").get(0).scrollHeight)},500);uploadFile(t)}let n=document.getElementById("drop-area");["dragenter","dragover","dragleave","drop"].forEach(i=>{n.addEventListener(i,t,!1)});["dragenter","dragover"].forEach(t=>{n.addEventListener(t,i,!1)});["dragleave","drop"].forEach(t=>{n.addEventListener(t,r,!1)});n.addEventListener("drop",u,!1)}function VideoCallProcess(){try{if(console.log("IsCameraPermission:"+IsCameraPermission),IsCameraPermission==!1){IsCameraPermission=JSBridge.getVideoPermission();console.log("IsCameraPermission:"+IsCameraPermission);return}}catch{}if(wsconn.connectionState!="Connected"&&wsconn.M!="Connected"){SendNotConnectAlert("you are not connect to signalr. For video call you must refresh page. Please refresh page.");return}beepring.play();beepring.volume=.3;$("#call-status-lbl").html("video calling to user...");isVideoCall=!0;console.log("calling user... ");$("#VideoCallModalLabel").html("Video Call");dt1=new Date;videoaudiotimer=setTimeout(timerDisplayFun,1e3);$(".partner")[0].style.display="none";localVideo.style.display="";remoteVideo.style.display="";var n=$(".def-mess.active").attr("data-id");if($("body").attr("data-mode")!=="idle"){alertify.error("Sorry, you are already in a call.  Conferencing is not yet implemented.",5);return}n!=myUserId?($.post("/Home/CallLogUpdate",{IsVideo:!0,Status:1,ToUser:n},function(){}),initializeUserMediaVideo(),wsconn.invoke("callUser",n,!0),dt1=new Date,setTimeout(timerDisplayFun,1e3),$("body").attr("data-mode","calling"),$("#VideoCallModal").modal("show"),$("#callstatus").text("Calling...")):alertify.error("Ah, nope.  Can't call yourself.",5)}function VoiceCallProcess(){try{if(console.log("IsMicrophonePermission:"+IsMicrophonePermission),IsMicrophonePermission==!1){IsMicrophonePermission=JSBridge.getAudioPermission();console.log("IsMicrophonePermission:"+IsCameraPermission);return}}catch{}if(wsconn.connectionState!="Connected"&&wsconn.M!="Connected"){SendNotConnectAlert("you are not connect to signalr. For voice call you must refresh page. Please refresh page.");return}beepring.volume=.3;beepring.play();$("#call-status-lbl").html("voice calling to user...");isVideoCall=!1;dt1=new Date;videoaudiotimer=setTimeout(timerDisplayFun,1e3);localVideo.style.display="none";remoteVideo.style.display="none";$("#VideoCallModalLabel").html("Voice Call");console.log("calling user... ");var n=$(".def-mess.active").attr("data-id");if($("body").attr("data-mode")!=="idle"){alertify.error("Sorry, you are already in a call.  Conferencing is not yet implemented.",5);return}n!=myUserId?($.post("/Home/CallLogUpdate",{IsVideo:!1,Status:1,ToUser:n},function(){}),initializeUserMediaVoice(),wsconn.invoke("callUser",n,!1),$("body").attr("data-mode","calling"),$("#ChatPmModal").modal("hide"),$("#VideoCallModal").modal("show"),$("#callstatus").text("Calling...")):alertify.error("Ah, nope.  Can't call yourself.",5)}function timerDisplayFun(){var r=new Date,n=r-dt1,t=Math.floor(n/1e3%60),i=Math.floor(n/6e4);document.querySelector(".recording-timer-display label").innerHTML=(i<10?"0":"")+i.toString()+":"+(t<10?"0":"")+t.toString();videoaudiotimer=setTimeout(timerDisplayFun,1e3)}function videoAndVoiceCallClick(){$(".social-media .fa-video-camera").on("click",function(){isVideoCall=!0;console.log("calling user... ");$("#VideoCallModalLabel").html("Video Call");$(".partner")[0].style.display="none";localVideo.style.display="";remoteVideo.style.display="";try{Android.loadCamera()}catch(t){alertify.error(t,5)}var n=$(".def-mess.active").attr("data-id");if($("body").attr("data-mode")!=="idle"){alertify.error("Sorry, you are already in a call.  Conferencing is not yet implemented.",5);return}n!=myUserId?(initializeUserMediaVideo(),wsconn.invoke("callUser",n,!0),$("body").attr("data-mode","calling"),$("#VideoCallModal").modal("show"),$("#callstatus").text("Calling...")):alertify.error("Ah, nope.  Can't call yourself.",5)});$(".social-media .fa-phone").on("click",function(){isVideoCall=!1;localVideo.style.display="none";remoteVideo.style.display="none";try{Android.loadMicrophone()}catch(t){alertify.error(t,5)}dt1=new Date;videoaudiotimer=setTimeout(timerDisplayFun,1e3);$("#VideoCallModalLabel").html("Voice Call");console.log("calling user... ");var n=$(".def-mess.active").attr("data-id");if($("body").attr("data-mode")!=="idle"){alertify.error("Sorry, you are already in a call.  Conferencing is not yet implemented.",5);return}n!=myUserId?(initializeUserMediaVoice(),wsconn.invoke("callUser",n,!1),$("body").attr("data-mode","calling"),$("#VideoCallModal").modal("show"),$("#callstatus").text("Calling...")):alertify.error("Ah, nope.  Can't call yourself.",5)})}function SetResultOfMessage(n,t,i){var f="",r,e,s,o,u,h,c,v,l,a;for(lmore==!1&&(f+="<ul>"),n.length>0&&(minId=n[n.length-1].id),minId>parseInt($(".contact.active").attr("minid"))&&(f+="<li class='lmore'><button class='btn btn-info'>load more<\/button><\/li>"),r=n.length-1;r>=0;r--)if(n[r].isFile==!1)f+=`<li class="`+(n[r].isReciver?"from-me":"from-other")+`" data-id="`+n[r].id+`" >`+(n[r].grpContactName?"<div class='name'>"+n[r].grpContactName+"<\/div>":"")+(n[r].body.indexOf(",;,")>=0?`<div class="card" data-id="`+(n[r].replyId?n[r].replyId:"")+`" >
