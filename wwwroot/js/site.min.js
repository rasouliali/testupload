var faTrashClicked=!1,SelectLi=!1,minId="0",lmore=!1;try{JSBridge.setUserId(myUserId)}catch{}var dt1,visible=!0;function showStatus(){$("#status-options").css("opacity","1"),$("#status-options").css("visibility","visible")}function searchContact(){var e=$("#search input").val();e?($(".new-search-mess").hide(),$(".new-search-mess").removeClass("new-search-mess").addClass("search-mess"),$(".new-search-mess").html(""),$.post("/Home/GetAccounts",{searchKey:e},function(e){if($(".def-mess").hide(),0==e.length)i=`<li class="contact new-search-mess">
                        ...
                        <div class="wrap">
                            <span class="contact-status `+a+'"></span>'+(e[t].profilePic?'<img src="/ProfilePics/thumbnails/'+e[t].profilePic+'" alt="" />':'<img src="/user-profile.png" alt="" />')+`<div class="meta">
                                <p class="name">`+e[t].userName+(data.id==myUserId?"<span class='badge text-info'>(You)</span>":"")+`<span></span></p>
                                <p class="preview"></p>
                            </div>
                        </div>
        ...
        `+`<div class="chat-pm-empty ">
            ...
            </li>`),$(".chat-pm ul").scrollTop($(".chat-pm ul").get(0).scrollHeight),setTimeout(function(){$(".chat-pm ul").scrollTop($(".chat-pm ul").get(0).scrollHeight)},500),uploadFile(t)},!1)}function VideoCallProcess(){try{if(console.log("IsCameraPermission:"+IsCameraPermission),0==IsCameraPermission)return IsCameraPermission=JSBridge.getVideoPermission(),void console.log("IsCameraPermission:"+IsCameraPermission)}catch{}var e;"Connected"!=wsconn.connectionState&&"Connected"!=wsconn.M?SendNotConnectAlert("you are not connect to signalr. For video call you must refresh page. For refresh page touch on ok button."):(beepring.play(),beepring.volume=.3,$("#call-status-lbl").html("video calling to user..."),isVideoCall=!0,console.log("calling user... "),$("#VideoCallModalLabel").html("Video Call"),dt1=new Date,videoaudiotimer=setTimeout(timerDisplayFun,1e3),$(".partner")[0].style.display="none",localVideo.style.display="",remoteVideo.style.display="",e=$(".def-mess.active").attr("data-id"),"idle"!==$("body").attr("data-mode")?alertify.error("Sorry, you are already in a call.  Conferencing is not yet implemented.").set({title:"ZN Vostok"}).set({delay:5e3}):e!=myUserId?($.post("/Home/CallLogUpdate",{IsVideo:!0,Status:1,ToUser:e},function(){}),initializeUserMediaVideo(),mySignalRListener.callUser(e,!0),dt1=new Date,setTimeout(timerDisplayFun,1e3),$("body").attr("data-mode","calling"),$("#VideoCallModal").modal("show"),$("#callstatus").text("Calling...")):alertify.error("Ah, nope.  Can't call yourself.").set({title:"ZN Vostok"}).set({delay:5e3}))}function VoiceCallProcess(){try{if(console.log("IsMicrophonePermission:"+IsMicrophonePermission),0==IsMicrophonePermission)return IsMicrophonePermission=JSBridge.getAudioPermission(),void console.log("IsMicrophonePermission:"+IsCameraPermission)}catch{}var e;"Connected"!=wsconn.connectionState&&"Connected"!=wsconn.M?SendNotConnectAlert("you are not connect to signalr. For voice call you must refresh page. For refresh page touch on ok button."):(beepring.volume=.3,beepring.play(),$("#call-status-lbl").html("voice calling to user..."),isVideoCall=!1,dt1=new Date,videoaudiotimer=setTimeout(timerDisplayFun,1e3),localVideo.style.display="none",remoteVideo.style.display="none",$("#VideoCallModalLabel").html("Voice Call"),console.log("calling user... "),e=$(".def-mess.active").attr("data-id"),"idle"!==$("body").attr("data-mode")?alertify.error("Sorry, you are already in a call.  Conferencing is not yet implemented.").set({title:"ZN Vostok"}).set({delay:5e3}):e!=myUserId?($.post("/Home/CallLogUpdate",{IsVideo:!1,Status:1,ToUser:e},function(){}),initializeUserMediaVoice(),mySignalRListener.callUser(e,!1),$("body").attr("data-mode","calling"),$("#ChatPmModal").modal("hide"),$("#VideoCallModal").modal("show"),$("#callstatus").text("Calling...")):alertify.error("Ah, nope.  Can't call yourself.").set({title:"ZN Vostok"}).set({delay:5e3}))}function timerDisplayFun(){var e=new Date-dt1,t=Math.floor(e/1e3%60),e=Math.floor(e/1e3/60);document.querySelector(".recording-timer-display label").innerHTML=(e<10?"0":"")+e.toString()+":"+(t<10?"0":"")+t.toString(),videoaudiotimer=setTimeout(timerDisplayFun,1e3)}function SetResultOfMessage(e,t,a){var i="";0==lmore&&(i+="<ul>"),(minId=0<e.length?e[e.length-1].id:minId)>parseInt($(".contact.active").attr("minid"))&&(i+="<li class='lmore'><button class='btn btn-info'>load more</button></li>");for(var s,o,l=e.length-1;0<=l;l--)if(0==e[l].isFile)i+='<li class="'+(e[l].isReciver?"from-me":"from-other")+'" data-id="'+e[l].id+'" >'+(e[l].grpContactName?"<div class='name'>"+e[l].grpContactName+"</div>":"")+(0<=e[l].body.indexOf(",;,")?'<div class="card" data-id="'+(e[l].replyId||"")+`" >
                      <div class="card-header" `+ForwardSenderId(e[l].body)+">"+e[l].body.split(",;,")[0].split(",@,")[0]+`
                      </div><label`+checkRtlOfText(e[l].body.split(",;,")[1])+">"+textToWrite(e[l].body.split(",;,")[1])+"</label></div>":"<label"+checkRtlOfText(e[l].body)+">"+textToWrite(e[l].body)+"</label>")+(e[l].isReciver?'<span class="'+IsSeen(e[l])+'">✓</span>':"")+'</li><li class="'+(e[l].isReciver?"from-me":"from-other")+`" >
                    <div class="date">`+getNiceDateTimeFormat(e[l].createdDate)+`</div>
                </li>`;else{var n=[],c=(n.push(e[l].body),"");0<=e[l].body.indexOf("^")&&(n=e[l].body.split("^"),c="");for(var d=0;d<n.length;d++){var r=n[d];0<n[d].indexOf(",;,")&&(r=n[d].split(",;,")[1],c=n[d].split(",;,")[0],n[d]=n[d].replace(",;,"," ")),i=(i+='<li class="'+(e[l].isReciver?"from-me":"from-other")+'" data-id="'+e[l].id+'" >')+(e[l].grpContactName?"<div class='name'>"+e[l].grpContactName+"</div>":""),c&&(i+='<div class="card" data-id="'+(replyid||"")+`" >
                      <div class="card-header " `+ForwardSenderId(c)+">"+c.split(",@,")[0]+"</div>"),IsImage(r)?i+='<label ><a href="#" data-url="/Home/GetFile?fileName='+r+'&fileType=up" class="img-tag"><div><img  class="fupload" src="/Home/GetFile?fileName='+r+'&fileType=up&th=true" /></div><div>'+SimpleFileName(r)+"</div></a></label>":IsAudio(r)?i+='<label><div><audio controls src="/Home/GetFile?fileName='+r+'&fileType=up" ></audio></div><div> '+SimpleFileName(r)+"</div></label>":IsVideo(r)?i+='<label><div><video controls  ><source src="/Home/GetFile?fileName='+r+'&fileType=up" ></video></div><div> '+SimpleFileName(r)+"</div></label>":IsPdfOrDoc(r)?i+='<label><a href="/Home/GetFile?fileName='+r+'&fileType=up" target="_blank" ><div><img class="docpdfimg" src="/Home/GetFile?fileName='+r.substring(0,r.lastIndexOf("."))+'.jpg&fileType=up&th=true" /></div><div>'+SimpleFileName(r)+"</div></a></label>":i+='<label><a href="/Home/GetFile?fileName='+r+'&fileType=up" target="_blank" ><div><img src="/img/file-icon.png" class="img-ico" /></div><div>'+SimpleFileName(r)+"</div></a></label>",c&&(i+="</div>"),i=(i+=(e[l].isReciver?'<span class="'+IsSeen(e[l])+'">✓</span>':"")+"</li>")+('<li class="'+(e[l].isReciver?"from-me":"from-other")+"\" ><div class='date'>"+getNiceDateTimeFormat(e[l].createdDate)+"</div></li>")}}lmore?($(".lmore").before(i),(1<$(".lmore").length?$($(".lmore")[1]):$(".lmore")).remove(),lmore=!1,$(".lmore").on("click",function(){lmore=!0;var e=$(".contact.active").data("id");$.post("/Home/GetContactMessage",{minId:minId,contactId:e},function(e){$(".loading").hide(),null==e||0==e.length?SetEmptyMessage():(SetResultOfMessage(e),$(".contact.active .text-bg-success").html("0"))})})):(i+="<li id='lastchat'></li></ul>",o=s="",o=($(".contact.active").hasClass("grp")?(s=30<(a=a||$(".contact.active .group-name").html()).length?a.substring(0,29)+"...":a,$(".contact.active .chat-bio")):(s=30<(a=a||$(".contact.active .name").html()).length?a.substring(0,29)+"...":a,$(".contact.active .company").text(),$(".contact.active .role"))).text(),a=`
		<div class="row">
			<div class="chat px-3">
        ...
        `+i+`
              ...
              <div class="progress">
                <div class="progress-bar" id="my-progress-bar" role="progressbar" style="width: 25%" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100">25%</div>
                        ...
                        </div>`;$("#search-contents").append(t),$(".content").on("click",function(){var e=$(this).data("id");$(".loading").show(),window.location.href="/?showInChat="+e})}})):($(".new-search-mess").hide(),$(".def-mess").show())}function getDuraton(e){var e=Math.round(e),t="";return 60<e&&(t=Math.floor(e/60)<10?"0"+Math.floor(e/60):Math.floor(e/60)),t+=(t?":":"00:")+(Math.floor(e%60)<10?"0"+Math.floor(e%60):Math.floor(e%60))}$("#btn-submit-delete").on("click",function(){var a="";$(".chat-pm li.selected").each((e,t)=>{a+=$(t).data("id")+","}),a=a.substring(0,a.length-1),$.post("/Home/DelMess",{messid:a},function(e){0<e.length&&0<e[0].id?(e=$(".contact.active").data("id"),mySignalRListener.sendBySignalRForDel(e,a+";;;"+userid),$.each(a.split(","),function(e,t){$(".chat-pm li[data-id="+t+"]").next(".chat-pm li").remove(),$(".chat-pm li[data-id="+t+"]").remove()})):alert("an error has occured!"),$("#deleteModal").modal("hide"),contact_must_be_selected(),deleteid=""})}),$("#btn-cancel-forward").on("click",function(){forwardid="",$("#forwardModal").modal("hide")}),$("#btn-submit-forward").on("click",function(){0<$("#forward-body .mess.active").length&&SendMessageProcess(!0),forwardid="",$("#forwardModal").modal("hide"),$(".chat-pm li.selected").removeClass("selected"),contact_must_be_selected()}),$("#forwardModal").on("hidden.bs.modal",function(){forwardid=""}),$(".def-mess").on("click",function(){var t,a;faTrashClicked?faTrashClicked=!1:(t=$(this).hasClass("grp"),fromNewChat=!1,$(".loading").show(),$(".contact").removeClass("active"),$(this).addClass("active"),a=$(this).data("id"),$(this).data("id"),$.post("/Home/GetContactMessage",{minId:"0",contactId:a,isGroup:t},function(e){$(".loading").hide(),null==e||0==e.length?SetEmptyMessage():(SetResultOfMessage(e),$(".contact.active .text-bg-success").html("0")),t?mySignalRListener.SeenMessegeForGroup("all","all",a):mySignalRListener.SeenMessege("all",a)}))}),$("#logout").on("click",function(){$("#logoutForm").submit()}),$("#search-btn").on("click",function(){$("#searchContactModal").modal("show")}),$("#searchContact").on("keydown",function(e){13==e.keyCode&&searchonmobile()}),$(".search-contact").on("click",function(){searchonmobile()}),$(".search-contact-reset").on("click",function(){$("#searchContact").val("")}),$(".logout").on("click",function(){$("#logoutForm").submit()}),$(".setting").on("click",function(){window.location.href="/Identity/Account/Manage/Index"}),$(".chatpage").on("click",function(){window.location.href="/"}),$(".chat-mess-new").on("click",function(){$.post("/Home/AllContact",{},function(e){for(var t="",a=0;a<e.length;a++){var i=e[a];t+='<div class="new-ch contact col-12'+(ul&&ul.find(e=>e.userId.toLowerCase()==i.id.toLowerCase())?" online":"")+'" data-id="'+i.id+`">
                            <div class="row">
                                <div class="col-12 mx-3 b-online">
                                    <div class="float-start mx-3 ">
                                        <img src="`+i.profilePic+`" class="img-pro" />
                                    </div>
                                    <div class="float-start text-start d-contents px-2">
                                        <div class="pro">
                                            <label class="name">`+(i.name||i.username)+(i.id==myUserId?"<span class='badge text-info'>(You)</span>":"")+`</label>
                                            <label class="company">`+i.companyName+`</label>
                                            <label class="role">`+i.position+`</label>
                                        </div>
                                    </div>
            ...
            video#localVideo {
                ...
                z-index:0;
                        ...
                        </div>`;$(".calls").html(t)})}),$(".newgroupCreate").on("click",function(){$.post("/Home/AllContact",{},function(e){for(var t,a="",i=0;i<e.length;i++){var s=e[i];a+='<option value="'+s.id+'">'+(s.name||s.username)+"</option>"}t=`<div class="content row">
