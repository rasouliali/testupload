        ...
        `+`<div class="chat-pm-empty ">
            ...
            <ul><li id='lastchat'></li></ul>`+`
              ...
              <div class="progress">
                ...
                </li>`;else{var c=[],r=(c.push(e[l].body),"");0<=e[l].body.indexOf("^")&&(c=e[l].body.split("^"),r="");for(var d=0;d<c.length;d++){var m=c[d];0<c[d].indexOf(",;,")&&(m=c[d].split(",;,")[1],r=c[d].split(",;,")[0],c[d]=c[d].replace(",;,"," ")),s=(s+='<li class="'+(e[l].isReciver?"from-me":"from-other")+'" data-id="'+e[l].id+'" >')+(e[l].grpContactName?"<div class='"+(e[l].isReciver?"float-end":"float-start")+"'><img src='"+n+"' class='img-pro' /></div><div class='"+(e[l].isReciver?"float-end":"float-start")+" name'>"+e[l].grpContactName+"</div>":""),r&&(s+='<div class="card" data-id="'+(replyid||"")+`" >
                      <div class="card-header " `+ForwardSenderId(r)+">"+r.split(",@,")[0]+"</div>"),IsImage(m)?s+='<label ><a href="/Home/GetFile?fileName='+m+'&fileType=up" class="js-smartPhoto img-tag"><div><img  class="fupload" src="/Home/GetFile?fileName='+m+'&fileType=up&th=true" /></div><div>'+SimpleFileName(m)+"</div></a></label>":IsAudio(m)?s+='<label><div><audio controls src="/Home/GetFile?fileName='+m+'&fileType=up" ></audio></div><div> '+SimpleFileName(m)+"</div></label>":IsVideo(m)?s+='<label><div><video controls  ><source src="/Home/GetFile?fileName='+m+'&fileType=up" ></video></div><div> '+SimpleFileName(m)+"</div></label>":IsPdfOrDoc(m)?s+='<label><a href="/Home/GetFile?fileName='+m+'&fileType=up" target="_blank" ><div><img class="docpdfimg" src="/Home/GetFile?fileName='+m.substring(0,m.lastIndexOf("."))+'.jpg&fileType=up&th=true" /></div><div>'+SimpleFileName(m)+"</div></a></label>":s+='<label><a href="/Home/GetFile?fileName='+m+'&fileType=up" target="_blank" ><div><img src="/img/file-icon.png"  /></div><div>'+SimpleFileName(m)+"</div></a></label>",r&&(s+="</div>"),s=(s+=(e[l].isReciver?'<span class="'+IsSeen(e[l])+'">✓</span>':"")+"</li>")+('<li class="'+(e[l].isReciver?"from-me":"from-other")+"\" ><div class='date'>"+getNiceDateTimeFormat(e[l].createdDate)+"</div></li>")}}}lmore?($(".lmore").before(s),(1<$(".lmore").length?$($(".lmore")[1]):$(".lmore")).remove(),lmore=!1,$(".lmore").on("click",function(){lmore=!0;var e=$(".contact.active").data("id");$.post("/Home/GetContactMessage",{minId:minId,contactId:e},function(e){$(".loading").hide(),null==e||0==e.length?SetEmptyMessage():(SetResultOfMessage(e),$(".contact.active .text-bg-success").html("0"))})})):(s+="<li id='lastchat'></li></ul>",o=i="",o=($(".contact.active").hasClass("grp")?(i=30<(a=a||$(".contact.active .group-name").html()).length?a.substring(0,29)+"...":a,$(".contact.active .chat-bio")):(i=30<(a=a||$(".contact.active .name").html()).length?a.substring(0,29)+"...":a,$(".contact.active .company").text(),$(".contact.active .role"))).text(),a=`
		<div class="row">
			<div class="chat px-3">
                            ...
                            <div class="row">
                                <div class="col-12 mx-3 b-online">
                                    <div class="float-start mx-3 ">
                                        <img src="`+s.profilePic+`" class="img-pro" />
                                    </div>
                                    <div class="float-start text-start d-contents px-2">
                                        <div class="pro">`+(s.isGroup?`
                                            <label class="group-name">`+(s.name||s.username)+(s.id==myUserId?"<span class='badge text-info'>(You)</span>":"")+`</label>
                                            <label class="chat-bio">`+s.companyName+"</label>":`
                                            <label class="name">`+(s.name||s.username)+(s.id==myUserId?"<span class='badge text-info'>(You)</span>":"")+`</label>
                                            <label class="company">`+s.companyName+`</label>
                                            <label class="role`+(s.position&&20<s.position.length?" ft-9":"")+'">'+s.position+"</label>")+`
                                        </div>
                                    </div>
