...
function SetEmptyMessage() {
    ...
    var userRole = $('.contact.active .role').text();
...
function SetResultOfMessage(res) {
            ...
            for (var j = 0; j < resArr.length; j++) {
                ...
                strLi += `<div class="card" data-id="` + (replyid ? replyid : "") + `" >
                      <div class="card-header">` +
                        ...
                        `</div>`;
                if (IsImage(filePath)) {
                    strLi += `<label><a href="/uploads/` + filePath + `" target="_blank"><div><img  class="fupload" src="/uploads/thumbnails/` + filePath + `" /></div><div>` + filePath +`<div></a></label>`;

                }
                else if (IsAudio(filePath))
                    strLi += `<label><div><audio controls src="/uploads/` + filePath + `" ></audio></div><div> ` + filePath +`</div></label>`;
                else
                    strLi += `<label><a href="/uploads/` + filePath + `" target="_blank" ><div><img src="/img/file-icon.png" class="img-ico" /></div><div>` + filePath + `</div></a></label>`;
    ...
    $('.pro-small').on('click', function () {
        $('.loading').show();
        window.location.href = "/Home/Profile?uid=" + $('.contact.active').data('id');
    });
    $('#ChatPmModal .close').on('click', function () {
        if (fromNewChat)
            window.location.href = "/";
