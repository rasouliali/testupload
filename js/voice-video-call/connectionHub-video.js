...
$(document).ready(function () {
    ...
    $(document).on('click', '.user', function () {
        ...
        if (UserId != myUserId) {
            // Initiate a call
            wsconn.invoke('callUser', UserId  /*{ "connectionId": targetConnectionId}*/);
...
var webrtcVoiceConstraints = { audio: true, video: false };
...
const partnerAudio = document.querySelector('.audio.partner');
var WOWZA_STREAM_NAME = null, connections = {}, localStream = null; remoteStream = null;
...
const initializeUserMediaVideo = () => {
    console.log('WebRTC: InitializeUserMedia: ');
    navigator.getUserMedia(webrtcConstraints, callbackUserMediaSuccess, errorHandler);
};
const initializeUserMediaVoice = () => {
    console.log('WebRTC: InitializeUserMedia: ');
    navigator.getUserMedia(webrtcVoiceConstraints, callbackUserMediaSuccess, errorHandler);
};
// stream removed
...
wsconn.on('incomingCall', (callingUser,isVideo) => {
    console.log('SignalR: incoming' + (isVideoCall?" video ":" voice ")+'call from: ' + JSON.stringify(callingUser));
    isVideoCall = isVideo;
    ...
    alertify.confirm(callingUser.username + ' is calling.  Do you want to' + (isVideoCall ? " video " : " voice ") +'chat?', function (e) {
        if (e) {
            // I want to chat
            if (isVideoCall == false)
...
const setUsername = (username,userid) => {
    ...
    wsconn.invoke("Join", username, userid).catch((err) => {
        consoleLogger(err);
